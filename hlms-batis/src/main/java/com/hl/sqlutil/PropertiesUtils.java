...
//				.getResourceAsStream("/config.properties");
	...
	public static Properties getjdbcProperties(){
		Properties prop = new Properties();
		String savePath = PropertiesUtils.class.getResource("/props/jdbc.properties").getPath();
		//以下方法读取属性文件会缓存问题
//		InputStream in = PropertiesUtils.class
//				.getResourceAsStream("/config.properties");
		try {
	...
	public static void modifyProperties(String key, String value) {
		try {
			...
			String path = PropertiesUtils.class.getResource("props/config.properties").getPath();
